...
The MIMIC subsets carry columns that are not model-ready as-is: numeric labs stored
as Decimal/object, datetime and id-like columns, string categoricals (gender, race),
constant columns, and labs with very high missing rates. This turns a raw frame into
an all-numeric feature matrix and reports what it did with every column.

Policy: object columns that parse as numbers become numeric; datetime and id-like
columns are dropped; numeric columns that are constant or missing above
`max_missing_rate` are dropped (imputing a mostly-absent lab is unreliable; a
companion `has_<lab>` indicator, if present, keeps the "was it measured" signal);
categorical columns are dropped if constant, too-missing, or above
`max_cat_cardinality` levels (one-hotting many rare levels is dispersive and the
sparse indicators can dominate a causal model); binary categoricals become a single
0/1 column, the rest are one-hot encoded.

Numeric missing values that survive are left as NaN and imputed later by the
train-fit `utils.standardize.Standardizer`, so no test information leaks in here.
"""
...
@dataclass
class FeatureReport:
    """How each column was routed. `dropped` maps column name to drop reason."""
